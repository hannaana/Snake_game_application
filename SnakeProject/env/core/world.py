...
class World(object):
    ...
    def init_snake(self):
        ...
        if not self.custom:
            ...
            start_position = (random.choice(list(range(SNAKE_SIZE, self.size[0] - SNAKE_SIZE))),
                              random.choice(list(range(SNAKE_SIZE, self.size[1] - SNAKE_SIZE))))
            # choose a random direction index
            start_direction_index = random.choice(list(range(len(DIRECTIONS))))
    ...
    def move_snake(self, action):
        ...
        if self.snake.alive:
            ...
            # next_snake_head = tuple(map(sum, zip(new_snake_head, tuple(self.DIRECTIONS[self.snake.current_direction_index]))))
            # if self.world[next_snake_head[0], next_snake_head[1]] == self.WALL:
            # if new_snake_head in self.world[1, :] or new_snake_head in self.world[:, self.size[1]-2] or new_snake_head in self.world[self.size[0]-2, :] or new_snake_head in self.world[:, 2]:
